...
df_total = psql.read_sql_query("SELECT * FROM gjf_total_data",db_connect)
df_pop_comp = psql.read_sql_query("SELECT * FROM gjf_pop_comp_data",db_connect)
df_pop_age = psql.read_sql_query("SELECT * FROM gjf_pop_age_data",db_connect)
df_employee_ratio = psql.read_sql_query("SELECT * FROM gjf_employee_ratio_data",db_connect)
...
def population_graph(date):
    #값
    value = int(df_total.loc[df_total['prd_de']==date,'population_total'].values[0])
    if round_check(value)!=0:
        value = str(round(value/1000)//10) + "만 " + str(round_check(value)) + '천명'
    else:
        value = str(round(value/1000)//10) + "만 명"
    #증감
    delta = int(df_total.loc[df_total['prd_de']==date,'population_total']) - int(df_total.loc[df_total['prd_de']==get_previous_year_date(date),'population_total'])
    #시각화
...
def marry_graph(date):
    if df_total.loc[(df_total['prd_de']==date),'num_married'].isna().any():
         st.metric(label='경기 혼인 수', value='---값 없음---')
    else:
        #값
        value = f"{int(df_total.loc[df_total['prd_de']==date,'num_married'].values[0]):,}건"
        #) + "건")
        #증감
        delta = int(df_total.loc[df_total['prd_de']==date,'num_married']) - int(df_total.loc[df_total['prd_de']==get_previous_year_date(date),'num_married'])
        #시각화
        st.metric(label='경기 혼인 수', value=value, delta=f"({delta:,})")
    
# 3. 경기 출생아 수 
def baby_graph(date):
    if df_total.loc[(df_total['prd_de']==date),'population_baby'].isna().any():
         st.metric(label='경기 출생아 수', value='---값 없음---')
    else:
        #값
        #value = str(int(df_total.loc[df_total['prd_de']==date,'population_baby'])) + "명"
        value = f"{int(df_total.loc[df_total['prd_de']==date,'population_baby'].values[0]):,}명"
        #) + "건")
        #증감
        delta = int(df_total.loc[df_total['prd_de']==date,'population_baby']) - int(df_total.loc[df_total['prd_de']==get_previous_year_date(date),'num_married'])
        #시각화
...
def mw_pop_graph(date):
    data_nm_pop = df_total.loc[df_total['prd_de']==date,['population_male','population_female']]
    data_nm_pop['남자인구수'] = round(int(data_nm_pop['population_male'])/10000,1)
    data_nm_pop['여자인구수'] = round(int(data_nm_pop['population_female'])/10000,1)
    data_nm_pop = data_nm_pop.loc[:,["남자인구수","여자인구수"]]
    data_nm_pop = data_nm_pop.melt()
    #color_map = {'population_male':'blue', 'population_female':'red'}
...
def employee_graph(date):
    if df_total.loc[(df_total['prd_de']==date),'population_economic'].isna().any():
        ...
        #값
        value = int(df_total.loc[df_total['prd_de']==date,'population_economic'].values[0])
        if round_check(value)!=0:
            value = str(round(value/1000)//10) + "만 " + str(round_check(value)) + '천명'
        else:
            value = str(round(value/1000)//10) + "만 명"
        #증감
        delta = int(df_total.loc[df_total['prd_de']==date,'population_economic']) - int(df_total.loc[df_total['prd_de']==get_previous_year_date(date),'population_economic'])
        #시각화
        st.metric(label='경기 경제활동인구', value=value, delta=f"({delta:,})")

# 8. 경기 실업자 수
def unemployee_graph(date):
    if df_total.loc[(df_total['prd_de']==date),'population_unemployed'].isna().any():
        ...
        value = int(df_total.loc[df_total['prd_de']==date,'population_unemployed'].values[0])
        if round_check(value)!=0:
            value = str(round(value/1000)//10) + "만 " + str(round_check(value)) + '천명'
        else:
            value = str(round(value/1000)//10) + "만 명"
        #증감
        delta = int(df_total.loc[df_total['prd_de']==date,'population_unemployed']) - int(df_total.loc[df_total['prd_de']==get_previous_year_date(date),'population_unemployed'])
        #시각화
        st.metric(label='경기 실업자 수', value=value, delta=f"({delta:,})")

# 9. 경기 사업체 수
def company_graph(date):
    if df_total.loc[(df_total['prd_de']==date),'num_company'].isna().any():
        st.metric(label='경기 사업체 수', value='---값 없음---')
    else:
        #값
        #value = str(int(df_total.loc[df_total['prd_de']==date,'num_company'])//10000) +"만 " + str(int(((int(df_total.loc[df_total['prd_de']==date,'num_company'])%10000)/1000)+0.5)) +"천명"
        #value = f"{round((int(df_total.loc[df_total['prd_de']==date,'num_company'].values[0])/10000)):,}만 "  + str(int(((int(df_total.loc[df_total['prd_de']==date,'num_company'])%10000)/1000))) +"천개"
        value = int(df_total.loc[df_total['prd_de']==date,'num_company'].values[0])
        if round_check(value)!=0:
            value = str(round(value/1000)//10) + "만 " + str(round_check(value)) + '천개'
        else:
            value = str(round(value/1000)//10) + "만 개"        
        #증감
        delta = int(df_total.loc[df_total['prd_de']==date,'num_company']) - int(df_total.loc[df_total['prd_de']==get_previous_year_date(date),'num_company'])
        #시각화
...
def recruit_graph(date):
    if df_total.loc[(df_total['prd_de']==date),'population_job_open'].isna().any() | df_total.loc[(df_total['prd_de']==date),'num_job_search'].isna().any() :
        st.metric(label='', value='---값 없음---')
    else:
        data_recruit = df_total.loc[df_total['prd_de']==date,['population_job_open','num_job_search']]
        data_recruit.rename(columns={'population_job_open': '구인인원(명)', 'num_job_search': '구직건수(건)'},inplace=True)
